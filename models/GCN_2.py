...
def get_laplacian_mat(edge_index,  num_node, normalization='sym'):  
...
def energy_torch(v1, L1):
    ...
    """
    # print(L1.shape)
    # print(L1.shape, v1.shape)
    # print(torch.dot(v1.t(), L1).shape)
    assert v1.shape[0] == L1.shape[0] == L1.shape[1]
    E = np.dot(np.dot(v1.T, L1), v1)
    E = np.diag(E)
    return np.sum(E)
...
class GCN(nn.Module):
    ...
    def forward(self, x, edge_index):
        ...
        fopen = open("./runs/data_gcn_l16.txt", "a")
